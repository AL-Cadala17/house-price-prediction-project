...
# --------------------------------
CSV_PATH = "dataset/house_dataset_clean.csv"
...
# --------------------------------
# 9) MODEL SAVING FOR NEW DATA
joblib.dump(lr,"models/lr_model.joblib")
joblib.dump(rf,"models/rf_model.joblib")
# --------------------------------

# 10) Optional: local custom input test using the shared helper
custom = {
    "Size_sqft": 2400, "Bedrooms": 3, "Bathrooms": 2,
    "YearBuilt": 2010, "Location": "City"
}

new_data = prepare_features_from_raw(custom)
print("=== CUSTOM DATA INPUT ===")
print(new_data)
print(f"LR Pred     : ${float(lr.predict(new_data)[0]):,.0f}")
print(f"RF Pred     : ${float(rf.predict(new_data)[0]):,.0f}")
