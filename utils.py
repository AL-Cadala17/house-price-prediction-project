...
def prepare_features_from_raw(record: dict) -> pd.DataFrame:
    """
    This function takes a single record of raw data input(Size_sqft, Bedrooms, Bathrooms, Location, YearBuilt), 
    and performs necessary feature engineering,one-hot, scaled feature row that matches training to prepare
    it for model prediction.
    ...
    bath = float(record.get("Bathrooms", 0.0))
    ...
    rooms_per_1000 = ((beds + bath) / (size / 1000.0)) if size else 0.0
    ...
    # one-Hot Encoding for location col
